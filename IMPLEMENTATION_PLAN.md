...
| Phase 3B: Electron Local Folder Provider                    | Partial   | `055846b feat: add local sync folder ipc` covers the IPC/foundation, and `47fd333 fix: harden local sync folder scanning` adds connection-ID validation plus resilient scans for symlinks, permission loss, and missing descendants. Full filesystem watching, overflow rescan behavior, and large-file provider verification still need a dedicated pass before this can be marked completed.                                              |
...
| Phase 3D: Read-Only UI, Projection Locks, and Unlink        | Partial   | Read-only helpers and mutation protection exist via `f7b4eb8` and `f09e44b`; projection locks exist via `1cbc573`; remove-from-app unlink orchestration exists via `40e9139 feat: unlink synced folders from app`. Keep-files conversion, full offline-state UI, and restart recovery need a focused completion pass.                                                                                                                       |
| Phase 4A: Presentation Readiness and Rehearsal              | Partial   | `ac0cc66 feat: add presentation readiness checks` adds readiness analysis foundation, `f48faf8 feat: gate media presentation by readiness` adds a readiness-gated start helper that excludes unavailable items and preserves snapshot identities, and `e4cbf72 fix: route media starts through readiness gate` wires File Explorer and Header presentation entry points through the gate. Rehearsal mode and full readiness UI remain open. |
| Phase 4B: Storage Management Dashboard                      | Partial   | `6539c4a`, `055a643`, `fd8bb81`, and `4f93b90` add accounting, usage UI, cleanup, and integrity scanning foundations; `1d6e1b8 feat: evict regenerable media cache by budget` adds LRU eviction for regenerable derived assets while respecting projection locks; `8acc7c9 feat: export redacted media storage diagnostics` adds diagnostics export foundation. Sync-cache eviction and full dashboard actions remain open.                 |
...
1. Complete or defer Phase 3B filesystem watching, overflow rescan, and large-file provider
   verification.
2. Complete Phase 3D keep-files conversion, offline-state UI, and restart-recovery behavior.
3. Complete Phase 4A readiness UI and rehearsal mode.
4. Complete Phase 4B sync-cache eviction and dashboard actions.
5. Run the full quality gate after the remaining non-roadmap items are complete.

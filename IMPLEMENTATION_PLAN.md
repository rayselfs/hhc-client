...
3. Extended desktop video compatibility through bundled VLC/libVLC playback and FFmpeg poster-only
   generation.
4. Read-only local and OneDrive directory synchronization.
5. Presentation readiness and storage management.

Each phase must be implemented, tested, and committed independently. Synchronization must not start
as a full implementation until its feasibility and data-model phases pass review.
...
| Phase 2B: Transcoding Data Model and Job Lifecycle          | Superseded | The derived-asset/job foundation remains, but new MP4 transcode jobs are no longer created. Legacy `transcoded-video` assets remain readable as compatibility fallback.                                                                                                                                                                                                                                                                                                                        |
| Phase 2C: Electron Native Transcoding                       | Superseded | User-selected FFmpeg/background transcode was replaced by bundled VLC/libVLC playback plus FFmpeg poster-only generation.                                                                                                                                                                                                                                                                                                                                                                      |
| Live Transcode Present                                      | Superseded | Live FFmpeg presentation was removed. Video projection is handled by embedded VLC/libVLC in Electron; Web remains browser-native only.                                                                                                                                                                                                                                                                                                                                                         |
...
| Desktop Video Engine                                        | Completed  | Implemented by bundled runtime resolution, `projection-vlc` integration, `video-poster` IPC, video-engine license notices, and package verification. Packaged runtime directories are present under `resources/video-engine`; actual release binaries must be placed there before distribution.                                                                                                                                                                                                |
...
| Preferences Integration                                     | Completed  | Media preferences are now split into subitems by `97861ba feat: refine media presenter controls and preferences`. User-facing FFmpeg configuration has been removed because desktop video playback/poster generation is an internal bundled engine concern. OneDrive and storage sections remain. LAN Remote preferences remain roadmap-bound and are intentionally not implemented yet.                                                                                                       |
| Final Quality Gate                                          | Completed  | `npm run lint`, `npm run typecheck`, `npx vitest run`, and `npm run build` pass. Current build reports PWA precache at 4.29 MiB / 5.00 MiB, font assets at 8.41 MiB / 10.00 MiB, and largest JS chunk at 1.42 MiB / 2.25 MiB.                                                                                                                                                                                                                                                                  |
...
None. Roadmap work remains limited to Recovery Center and LAN Mobile Remote Control for the
...
- Do not reintroduce unconditional PDF.js, speech, Transformers, VLC, or FFmpeg loading at startup.
...
- Run long-lived import, thumbnail, poster, and sync work through one persistent job system.
- Treat generated thumbnails, video posters, and legacy transcoded media as derived assets with a
  shared lifecycle.
- Do not create new video transcode derivatives. FFmpeg is bundled only for poster generation.

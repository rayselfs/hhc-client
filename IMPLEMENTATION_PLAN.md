...
| Phase 3B: Electron Local Folder Provider                    | Partial   | `055846b feat: add local sync folder ipc` covers the IPC/foundation. Full filesystem watching, rescan behavior, permission-loss handling, and large-file provider verification still need a dedicated pass before this can be marked completed.        |
...
| Phase 3D: Read-Only UI, Projection Locks, and Unlink        | Partial   | Read-only helpers and mutation protection exist via `f7b4eb8` and `f09e44b`; projection locks exist via `1cbc573`. Unlink options, full offline-state UI, and restart recovery need a focused completion pass.                                         |
...
1. Complete or defer Phase 3B Electron local folder provider behavior beyond the IPC foundation.
2. Complete Phase 3D unlink/offline-state/restart-recovery behavior.

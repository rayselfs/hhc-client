...
- [ ] Add a failing Home structure test that expects `Font`, `Paragraph`, `Position`, and
      `Arrange` groups in document order.
- [ ] Run the focused tests and verify the current `h-24` and ungrouped markup fail.
- [ ] Add the minimal local group shell:
...
- [ ] Add localized labels for Font, Paragraph, Position, Arrange, Insert, Background, Adjust,
      and Size.
- [ ] Change the frame height to `h-28`; leave collapse behavior unchanged.
...
- [ ] Add failing assertions that the Home command surface has
      `overflow-x-auto overflow-y-hidden` and each group has a fixed `shrink-0` boundary.
- [ ] Add a failing assertion that no Home arrange container uses `flex-wrap`.
- [ ] Run focused tests and verify the current flexible row fails.
- [ ] Recompose Home/Text as:
...
- [ ] Place font controls and paragraph controls in two explicit rows.
- [ ] Place X/Y/width/height in a fixed 2×2 grid.
- [ ] Place alignment and distribution actions in a fixed 4-column grid without wrapping.
- [ ] Run the font-size and numeric-control behavioral tests plus the new structure tests.
- [ ] Commit.
...
- [ ] Add failing tab tests that click Insert and Design and expect their named groups.
- [ ] Add a selected-image test that opens Picture Format and expects Adjust, Arrange, and Size
      groups.
- [ ] Run the focused tests and verify current one-row panels fail.
- [ ] Wrap existing controls in the shared surface and group shell; keep all existing handlers.
- [ ] Use compact vertical icon commands for Insert and fixed rows for Picture controls.
- [ ] Run the complete presentation workspace page tests and commit.

### Task 4: Visual and Full Verification

- [ ] Start the browser renderer and open an editable presentation with a selected text element.
- [ ] Capture a screenshot of `presentation-ribbon-frame` and compare two-row density, group
      separators, field prominence, and command order against the supplied PowerPoint reference.
- [ ] Remove one unnecessary visual accent if the result reads as decorative rather than native.
- [ ] Run all presentation workspace tests.

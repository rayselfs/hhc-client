...
### Enabled client integration and release candidate

- The app supervisor now starts and cancels the personal worker with the active authenticated owner. Initial collection failures remain visible with retry even before a root exists. Cloud root identity, icon and edit/drag protections are enabled; per-item status comes from durable revisions/outbox failures. Root remains a valid copy/upload destination.
- Account departure hides and flushes private editor sessions, fences late opens and projection starts, and closes projection when retained private media becomes inaccessible. Pending data stays in the owner's local database. Personal folder favorites use a catalog-only transaction so they cannot overwrite queued metadata changes.
- Portable decks validate rich text, paragraph metrics, colors, shapes and themes in addition to graph and embedded images. Possible missing fonts show a fallback notice without rewriting original font settings. A clean file at its acknowledged cloud revision reuses its local snapshot instead of downloading its own upload again.
- Full local verification before the last status/font refinements: 274 test files / 3,197 tests passed; lint, desktop build and web build passed. Browser E2E: 49 passed, one existing skip. The new two-context test covers offline rename conflict, exact receipt replay after lost response, and explicit local backup before accepting the cloud version. These contexts use a deterministic mocked Account/Asset API; they are not production multi-device acceptance.
- Renderer minification uses the existing esbuild option. This retains the 5 MiB precache budget (2.72 MiB measured) without adding dependencies or increasing limits. Updated E2E database fixtures open the existing DB without requesting a lower schema version.
- Release candidate is 2.5.0, including the previously merged camera feature. Remaining gates: final checks after refinements, Presenter PR/CI/merge and desktop/web release, authenticated cloud CRUD/deck/large-file smoke, and actual macOS/Windows device acceptance. Preserve task worktrees until those acceptance gates are complete.
- Final local refinement checks passed: 275 test files / 3,199 tests, lint, desktop build with node/web typechecks and unchanged bundle budgets. Initial-root error UI and font fallback measurement each have a runnable regression check.
...
- Published artifact readback: downloaded the actual GitHub `hhc-presenter-2.5.0-arm64-mac.zip` and matched SHA256SUMS (`bb0235305f00ed7fc8bcc3ae93bce28a127b1a970f75acbafd41a087c6634fbb`). Launched that extracted release binary using the isolated authenticated QA profile. It downloaded preserved item `01819a7a-bc9a-4a23-902e-376d4aa3a7c3`, revision 20, and matched the browser's 2,472-byte SHA-256 `a530f0c6daa03d6e76c0ae70de2f78073fb428b8c2db632154dd0edef1ea39f4`. Receipt: `/tmp/hhc-personal-published-native-evidence.json`.

...
| 6     | `rayselfs/hhc-client`                        | `6935ad5fa5ccb668febb578c2fbe5b3af5f4a8ff` on `fix/media-projection-merge-readiness` | [PR #6](https://github.com/rayselfs/hhc-client/pull/6) remains open; closure review and final-head hosted CI/package jobs are pending. Local unit, lint, typecheck, build/bundle, browser E2E, and macOS arm64 packaged VLC/FFmpeg smoke are green. | Merge into `main` is permitted only after reviewed closure changes and green final-head gates. Tagging, unsigned artifact publication, and deployment remain separate approval checkpoints. | Media projection closure is technically complete locally; hosted macOS arm64 and Windows x64 packaged jobs, hosted review, and final policy reconciliation remain required before merge. Intel macOS is not a supported target. | Current production web/desktop remains the prior main release; no client tag, release publication, deployment, or pilot mutation is approved. |

The current client release-code anchor is
`6935ad5fa5ccb668febb578c2fbe5b3af5f4a8ff`. A later commit that changes only release-policy,
security-review wording, or verification evidence is not a different executable code candidate. Before merge, the
